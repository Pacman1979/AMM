...
describe('AMM', () => {
  ...
  describe('Swapping tokens', () => {
      ...
      expect(await amm.totalShares()).to.equal(tokens(100))
	    ...
	    estimate = await amm.calculateToken1Swap(tokens(1))
	    console.log(`ESTIMATED Token2 amount Investor1 will receive from swap: ${ethers.utils.formatEther(estimate)}\n`)
	    ...
	    // Check investor1 balance after swap
	    // newEstimate = await ethers.utils.formatEther(estimate)
	    newBalance = await token2.balanceOf(investor1.address)
	    console.log(`NEW Investor1 Token2 balance after swap: ${ethers.utils.formatEther(newBalance)}\n`)
	    ...
	    newEstimate = await ethers.utils.formatEther(estimate)
	    newBalance = await token2.balanceOf(investor1.address)
	    // newTotal = await(newBalance + newEstimate)
	    // console.log(`NEW Investor1 Token2 balance after swap: ${ethers.utils.formatEther(newTotal)}\n`)
